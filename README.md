...
Note: Input must be pre-sorted! Typically used after sort.
...
--method	max	Distance calculation method: max (maximum difference) or sum (sum of differences)
...
parse: I/O-bound; SSD/NVMe significantly improves throughput

sort: CPU and memory-bound; set --nproc to number of cores, adjust --memory based on available RAM

dedup: Larger --max-mismatch increases window size and memory usage; 3-5bp is usually sufficient

Release builds enable LTO and opt-level=3; always use cargo build --release for production
...
├── src/
│   ├── main.rs             # Entry point
│   ├── cli/mod.rs          # CLI definitions (clap)
│   ├── format/mod.rs       # Data structures (Alignment, PairRecord, PairType)
│   ├── header/mod.rs       # Pairsam header operations
│   ├── io/mod.rs           # I/O (gzip support)
│   ├── parse/mod.rs        # SAM→pairsam conversion core logic
│   ├── sort/mod.rs         # Multi-threaded sorting (rayon + k-way merge)
│   └── dedup/mod.rs        # PCR deduplication (online sliding window algorithm)
└── README.md
License
MIT
